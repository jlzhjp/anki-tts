...
func TestBatchProgressShowsRetryAndSummary(t *testing.T) {
	...
	updated, _ := model.Update(ankitts.ProgressEvent{
		Kind: ankitts.ProgressRetrying, Index: 0, NoteID: 1, Operation: ankitts.OperationSynthesize,
	...
	updated, _ = model.Update(batchFinishedMsg{result: ankitts.BatchResult{Items: []ankitts.ItemResult{{NoteID: 1}}}})
...
func plannedNotes(count int, overwrite bool) []ankitts.PlannedNote {
	...
	for index := range notes {
		notes[index] = ankitts.PlannedNote{Index: index, Note: anki.Note{ID: int64(index + 1), ModelName: "Basic"}, SourceText: "hello", WillOverwrite: overwrite}
	}
	return notes
}
